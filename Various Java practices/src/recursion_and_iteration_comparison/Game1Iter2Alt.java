...
public class Game1Iter2Alt {
	...
	public boolean won(int firstScore, int secondScore) {
		...
		return ((firstScore >= 15) && ((!winByTwo) || (firstScore >= secondScore + 2)));
	}

	/**
	 * allows player with first score to continue serving, until player has won
	 * game or lost volley
	 * 
	 * @parms probWinVolley is probability that player with first score wins
	 *        volley firstScore, secondScore are scores of the two players/teams
	...
	public String playUntilWin(double probWinVolley) {
		...
		while (true) {
			...
			// stop if B has won
			if (won(BScore, AScore)) {if (printGame)
